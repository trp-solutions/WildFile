...
var WildFile = (function(){
	...
	class Filelist {
		...
		reset(){
			...
			this.files = {};
			if(typeof this.handler.listreset == 'function'){
				this.handler.listreset(this, old_files);
			}
		}
		...
		add_file(raw_file){
			...
			chunked_file.complete.then(chunked_file=>{
				...
				if(typeof this.handler.complete == 'function'){
					this.handler.complete(entry);
				}
			})

			if(typeof this.handler.add == 'function'){
				this.handler.add(entry);
			}
		}
