...
var WildFile = (function(){
	...
	function try_call(func, ...arguments){
		if(typeof func == 'function') func(...arguments);
	}
	...
	class Filelist {
		...
		static get(key, reset_list){
			if(!this.lists[key]){
				this.lists[key] = new Filelist(key);
			} else if(reset_list){
				this.lists[key].reset();
			}
		...
		reset(){
			let old_files = this.files;
			for(let id in old_files){
				old_files[id].chunked_file.abort();
			}
			this.files = {};
			try_call(this.handler.listreset, this, old_files);
		}
		...
		add_file(raw_file){
			...
			if(typeof this.handler.progress == 'function'){
				chunked_file.register_progress_callback((value,max)=>this.handler.progress(entry, value, max));
			}

			chunked_file.complete.then(
				chunked_file=>{
					entry.status = 'complete';
					try_call(this.handler.complete, entry);
				},
				message=>try_call(this.handler.failure, entry, message)
			)

			try_call(this.handler.add, entry);
		}

		add(input){
			for(let raw_file of input.files){
				this.add_file(raw_file);
			}
			input.value = null;
		}

		upload(url){
			const complete_promises = [];
			for(let id in this.files){
				let entry = this.files[id];
				if(typeof entry == 'object' && entry.status == 'pending'){
					entry.status = 'uploading';
					complete_promises.push(entry.chunked_file.complete);
					entry.chunked_file.upload(url);
					if(typeof this.handler.upload == 'function'){
						this.handler.upload(entry);
					}
				}
			}
			if(typeof this.handler.listfinish == 'function'){
				Promise.allSettled(complete_promises).then(results=>{
					let successful_files = 0;
					results.forEach(result=>{
						if(result.status == 'fulfilled'){
							successful_files += 1;
						}
					})
					this.handler.listfinish(this, successful_files, results.length);
				})
			}
		}
	}
	...
	var exportobj = {
		...
		list: (key,reset_list) => Filelist.get(key,reset_list),
	};
